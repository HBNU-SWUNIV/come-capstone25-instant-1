...
namespace AmazingAssets.CurvedWorld.Editor
{
    ...
    public class CurvedWorldControllerEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            using (new EditorGUIHelper.GUIEnabled(_target.isActiveAndEnabled))
            {
                ...
                using (new EditorGUIHelper.EditorGUILayoutBeginHorizontal())
                {
                    ...
                    {
                        EditorUtilities.CallbackOpenCurvedWorldEditorWindowControllers(_target);
                    }
                }
        ...
        void Draw_RotationCenter()
        {
            ...
            if (_target.bendType == CurvedWorld.BendType.SpiralHorizontalDouble_X ||
                _target.bendType == CurvedWorld.BendType.SpiralHorizontalDouble_Z ||
                _target.bendType == CurvedWorld.BendType.SpiralVerticalDouble_X ||
                _target.bendType == CurvedWorld.BendType.SpiralVerticalDouble_Z)
            {
                label += " #1";
            }
        ...
        {
            //Axis
            using (new EditorGUIHelper.GUIEnabled(bendRotationAxisType.enumValueIndex != (int)CurvedWorld.CurvedWorldController.AxisType.Transform))
            {
